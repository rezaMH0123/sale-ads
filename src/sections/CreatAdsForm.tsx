...
import { createAdsController } from "../controllers/adsController";
...
const CreatAdsForm = ({ position }: CreateAdsFormProps) => {
  ...
  const onSubmit = async (data: FormValues) => {
    ...
    try {
      const res = await createAdsController(adsData);
  ...
  return (
    <form onSubmit={handleSubmit(onSubmit)} className="space-y-4 p-4">
      ...
      >
        {loading ? <span>صبر کنید...</span> : <span>ثبت</span>}
      </button>
    </form>
  );
};

export default CreatAdsForm;
